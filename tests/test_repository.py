...
def test_repository_repairs_malformed_spaced_booking_flight_numbers_on_startup(tmp_path: Path) -> None:
    settings = Settings(
        data_dir=tmp_path / "data",
        config_dir=tmp_path / "config",
        templates_dir=Path("app/templates"),
    ...
    try:
        ...
        connection.execute(
            ...
            INSERT INTO bookings (
                booking_id, source, trip_instance_id, route_option_id, data_scope, airline, origin_airport, destination_airport,
                departure_date, departure_time, arrival_time, arrival_day_offset, fare_class, stops, flight_number, booked_price,
                record_locator, booked_at, booking_status, match_status, raw_summary, candidate_trip_instance_ids, auto_link_enabled,
                resolution_status, notes, created_at, updated_at
            ) VALUES (
                'book_bad', 'gmail', 'inst_1', '', 'live', 'Southwest', 'LAX', 'SFO',
                '2026-05-11', '06:13', '07:44', 0, 'economy', 'nonstop', '242 6', 108.4,
                'ABC123', '2026-04-01T12:00:00+00:00', 'active', 'matched', '', '', 1,
                'resolved', '', '2026-04-01T12:00:00+00:00', '2026-04-01T12:00:00+00:00'
            )
            """
        )
        connection.commit()
    ...
    repaired_booking = next(item for item in repository.load_bookings() if item.booking_id == "book_bad")
    assert repaired_booking.flight_number == "WN 2426"
