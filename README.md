...
- each rule is one-way and can contain multiple ranked fallback slots
- the app generates future trip instances and one tracker per ranked slot
- you turn on `Track prices` in Google Flights manually
- you import Google Flights `.eml` alerts into the app
- the app rolls those slot signals into `set up`, `act now`, `booked`, and `rebook` decisions
...
1. Open `Rules` and save your recurring flight rules.
   Use multiple one-way rules when your outbound and return preferences differ, and rank backup slots inside each rule.
2. Open `Trackers` and use the generated Google Flights links.
3. In Google Flights, turn on `Track prices` for each slot you care about.
